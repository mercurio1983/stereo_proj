...
int main(int argc, char *argv[])
{
  ...
  GLuint overlaydrawTexture_texture[2];			// This is a handle to our texture object
  ...
  if ( ( ManeqImage[0] = SDL_LoadBMP( "left.bmp" ) ) && 
       ( ManeqImage[1] = SDL_LoadBMP( "left.bmp" ) ) )
    {
      ...
      glDisable(GL_BLEND);
      //gdc1394camera_tlDisable(GL_DEPTH_TEST);
  ...
  */
  ...
  GLuint video_texture[4];			// This is a handle to our texture object
...
//avt_singleview.c:128:
  ...
  while ( ! done ) {
    ...
    if( drawVideo == 1)
      {
	drawTexture2(screen->w, screen->h, device_width, device_height, stretch, video_texture, frames[0]->image, frames[1]->image, portion);
