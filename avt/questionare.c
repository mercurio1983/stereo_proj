...
void drawTexture2(int width, int height, int device_width, int device_height,float s, GLuint* tex, unsigned char* data_left, unsigned char* data_right, float portion)
{
  ...
  //  else{set_tex_value = 1;}
    ...
    */    
	...
	glBegin(GL_QUADS);	
      glTexCoord2f(0.3f, 0.0f); glVertex2f((0.0f)+(GLfloat)left_border_split, 0.0f);
      glTexCoord2f(0.7f, 0.0f); glVertex2f((GLfloat)right_border_split, 0.0f);
      glTexCoord2f(0.7f, 1.0f); glVertex2f((GLfloat)right_border_split, (GLfloat)height);
      glTexCoord2f(0.3f, 1.0f); glVertex2f((0.0f)+(GLfloat)left_border_split, (GLfloat)height);
    glEnd();
    /*
    glBegin(GL_QUADS);	
    ...
    */
	...
	//glTexImage2D(GLenum  target,  GLint  level,  GLint  internalFormat,  GLsizei  width,  GLsizei  height,  GLint  border,  GLenum  format,  GLenum  type,  const GLvoid *  data);
	glTexImage2D(GL_TEXTURE_2D,  0,  GL_RGB,  device_width,  device_height,  0,  GL_RGB,  GL_UNSIGNED_BYTE, data_right);
